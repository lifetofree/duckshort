...
describe('StatsView — stats display', () => {
  ...
  it('limits displayed countries to statsLimit', () => {
    const manyCountries: StatsData = {
      ...SAMPLE_STATS,
      countries: Array.from({ length: 10 }, (_, i) => ({ country: `C${i}`, count: 10 - i })),
    }
    renderWithProviders(<StatsView {...defaultProps({ stats: manyCountries, statsLimit: 5 })} />)
    // Only first 5 should appear (statsLimit = 5) — C5 appears in NeonHeatmap but not in countries list
